...
- `source complete; runtime-gated`: Marketing ledger worker. Owner role: Marketing ledger worker. Scope: durable run ledger, idempotency key registry, dry-run summaries, and complete source/window snapshot accounting. Source/status evidence resolves the generic ledger proof; remaining blocker is `[MISSING: deploy/apply updated Marketing ledger migration containing complete_snapshot]`.
- `partially resolved; blocked`: marketplace producer worker. Owner role: marketplace producer worker. Scope: prove protected repeatable replay producer completeness for marketplace windows without temporary `/tmp` exports. Bazos source evidence resolves the generic Bazos eligibility mapping into exact fail-closed blockers; remaining producer blockers include `[MISSING: marketplace producer guarantee that replay window is complete and repeatable]`, `[MISSING: Bazos paid order history source]`, `[MISSING: Bazos persisted order item replay source]`, `[MISSING: Bazos order item ingestion contract]`, and `[MISSING: Bazos runtime internal replay token env accepted by /internal/bazos/order-affinity/replay-candidates]`.
