...
export class WarehouseAvailabilityService {
  ...
  async getBatchAvailability(
    ...
    const warehouseRows = await this.fetchWarehouseAvailability(productIds, warehouseIds);
    const rowsByProductId = new Map(warehouseRows.map((row) => [row.productId, row]));
