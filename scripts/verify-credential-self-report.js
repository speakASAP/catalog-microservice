...
check('reporter probes warehouse, not catalog or /health', () => {
  ...
  assert(!/url:.*catalog-microservice/.test(text), 'probes catalog, whose guard grants read by header not by role');
});
...
check('principal string matches auth exactly', () => {
  ...
  assert(
    text.includes("'svc-catalog--warehouse@internal.alfares.cz'"),
