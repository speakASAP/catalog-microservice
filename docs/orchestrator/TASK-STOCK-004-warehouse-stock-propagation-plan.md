...
- Completed: Bazos no-draft catalog status now returns an empty read-only status envelope instead of an unhandled dependency error; deployed at `bazos-service@804bf75`.
...
- Completed source: Catalog read-only channel-status smoke mode added and deployed; hosted-Auth validation now proves Warehouse, FlipFlop, Allegro, Bazos, Aukro, Bazos-account, and Aukro-account status paths return HTTP `200` for the target product without channel mutations.
