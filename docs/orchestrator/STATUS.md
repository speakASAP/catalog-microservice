2026-07-03: Goal 24 W1 Allegro protected replay producer integrated. Allegro `main` now includes `37a5add docs: finalize goal 24 allegro producer handoff` on top of the hardened producer source. `GET /internal/allegro/order-affinity/replay-candidates` is documented as protected by internal token plus `x-service-name=marketing-microservice`, emits aggregate-safe `marketplace.order_affinity_candidate.v1` snapshots, uses bounded `windowEnd`, deterministic `orderDate/id` ordering, opaque `cursorAfter`, and `completeSnapshot` semantics. Allegro-owned temporary-export replacement is resolved. Remaining affinity gates are Marketing marketplace parser/token mapping, durable ledger/idempotency/source-window proof, owner-reviewed publish window, docs-rag indexed context, and non-Allegro scheduled matrix gaps.
2026-07-03: Goal 24 integration ownership reconciled after runtime smoke, retention policy, and bundle contract merges. Original Codex thread `019f2683-0cac-7ec0-b4cf-8fe83e07a74e` is the Catalog docs/status integration owner. Marketing ledger worker owns the durable ledger/idempotency/source-window proof blocker, and marketplace producer worker owns the protected repeatable replay/completeness blocker. W1 Allegro producer handoff is integrated; unresolved handoffs remain `[MISSING: Marketing durable run ledger proving a complete source/window snapshot]`, `[MISSING: Marketing parser support for marketplace-owned replay source envelopes]`, and non-Allegro scheduled matrix producer gaps.
