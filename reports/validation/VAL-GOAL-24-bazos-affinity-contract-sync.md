...
- Bazos runtime follow-up records Marketing dry-run failing closed with HTTP 401 because the runtime accepted-token environment is missing on the Bazos side.
- Marketing `docs/orchestrator/STATUS.md` records marketplace parser support and durable complete-snapshot ledger proof at source/status level, while retaining runtime/integration blockers for deploying/applying the updated `complete_snapshot` ledger migration and producer completeness.
...
| Marketing parser/ledger | source complete; runtime-gated | Marketing worker | Parser support, completeSnapshot ledger proof, idempotency registry | deploy/apply updated ledger migration | Marketing focused tests/build/diff check from status | Catalog keeps runtime blocker |
...
| Runtime Bazos scheduling | blocked | Integration owner | Marketing pod dry-run and recurring schedule decision | Bazos paid source, item replay, ingestion contract, token acceptance, owner window | Not run | Keep inactive |
...
Merge order: Bazos evidence commit `31c245c`, Marketing source/status evidence, Catalog docs sync branch, future runtime token/order-source work only after owner approval.
