...
FlipFlop `902f637 docs: stabilize goal24 auth actor verifier` preserves the owner-approved discount/price fixture path and auth actor verifier stabilization for a future exact linked paid/provider smoke: checkout-authoritative total `<= 300 CZK` after recalculating the fixed discount to `2117.58 CZK`, one use, short expiry, and Goal 24 correlation. FlipFlop source now adds a narrow Goal 24 bundle-preserving fixture gate requiring `goalId=GOAL24-paid-provider-fixture-20260704`, fixed `2117.58 CZK`, `maxUses=1`, unused code, the approved target bundle, exact component ids, and checkout-authoritative final total `<=300 CZK`; it has not been deployed or runtime-quoted in Catalog evidence.
...
- FlipFlop `902f637 docs: stabilize goal24 auth actor verifier`: owner-approved server-validated discount/price fixture path remains documented for a future exact linked paid/provider smoke, with fixed `2117.58 CZK` discount required to keep the tax-inclusive checkout-authoritative total at `300 CZK`; source still contains the bundle-preserving fixture gate, uses the already guarded validation discount value, and has verifier alignment for the payment-result URL builder, but deploy/runtime quote evidence remains missing and the prior approval window was expired.
- Payments `13f6182 test: cover fiobanka refund upload gate`: the Fiobanka refund upload gate is covered and the narrowed refund runtime packet remains source-controlled, but completed-transfer refund/reversal execution and exact smoke identities remain missing.
...
Current committed upstream heads consumed: Orders `e3f6e18`, Payments `13f6182`, Warehouse `9f8b438`, FlipFlop `902f637`, Catalog pre-change `9eea93b`. FlipFlop auth actor verifier stabilization and payment-result verifier alignment are consumed as source/docs evidence only; Warehouse reserved/fulfilled/return/partial/timeout operation narrowing is consumed; dirty runtime/deploy follow-up evidence is not consumed until it is pushed and validated.
...
- FlipFlop: clean `main...origin/main` at `902f637 docs: stabilize goal24 auth actor verifier`; Catalog consumes the pushed source/docs auth actor verifier stabilization, guarded-discount safety fix, channel cleanup URL ownership narrowing, and payment-result verifier alignment; deployed bundle-preserving fixture evidence and sanitized payment URL runtime config readback remain missing.
...
- Orders `a7a6947 docs: record synthetic returned shipment fixture`: records that Orders can express rollback choreography only after Payments proves provider refund/cancel/reversal and the owner-approved runtime packet defines Orders cancellation actor/reason/side-effect acknowledgements plus Warehouse `9f8b438` current-head cleanup/delivery documentation retaining `3043cad` cleanup semantics; it also source-verifies current FlipFlop central Orders UUID propagation into Payments create calls.
...
- Orders `a7a6947 docs: record synthetic returned shipment fixture`: merged source/docs evidence that Orders can express completed, failed, and cancelled payment-status effects, owner-approved cancellation cleanup through the Warehouse `9f8b438` current-head documentation retaining the `3043cad` release/cancel/return matrix, and current FlipFlop central Orders UUID propagation into Payments create calls; it still requires Payments-owned provider refund/cancel/reversal proof before any paid rollback.
...
Current clean heads consumed from the orchestrator handoff: Catalog `9eea93b`, Orders `e3f6e18`, Payments `13f6182`, Warehouse `9f8b438`, and FlipFlop `902f637`. Catalog treats them as readiness/status inputs only.

Resolved/narrowed by this reconciliation: stale Catalog references to Orders `9f89e74`, Payments `ae51d7e`, and FlipFlop `fbe585c` were superseded by the clean heads above. Remaining hard blockers are unchanged: renewed execution window, named owners/actor/token path, deployed bundle-preserving fixture evidence, sanitized payment URL config readback, Warehouse stock window/max quantity, provider callback/refund evidence, and concrete future payment/order/provider idempotency keys.
