...
- Payments: clean `main...origin/main` at `2563864 Merge goal24 owner approved rollback packet`. This supersedes `1458ffe`, `e713bc4`, and `ea4fc71` by retaining selected-provider callback evidence, the provider rollback packet, Fiobanka rollback boundary, source-only refund/cancel rollback plan, and source-level Fiobanka HMAC signature hardening and a prepared owner-approved rollback packet while still blocking provider-specific refund/cancel/reversal execution approval, runtime `FIO_BANKA_WEBHOOK_SECRET` configuration/deploy verification, official/native Fio Banka callback signature contract if provider-authentic bank-originated signatures are required, Orders/Warehouse mutation, deploy, migration, and secret output.
...
- Payments `2563864 Merge goal24 owner approved rollback packet`: pushed main records pending Fiobanka QR creation, owner-approved synthetic completed callback through `/webhooks/fiobanka`, selected-provider callback evidence, provider rollback packet evidence, Fiobanka pre-completion rollback boundary, source-only refund/cancel rollback plan, and source-level Fiobanka HMAC signature hardening, and a prepared owner-approved rollback packet that remains runtime-hard-stopped; Catalog treats this as dependency-gated evidence because it still records `[RESOLVED/NARROWED: owner-confirmed real Fiobanka 1 CZK bank transfer for variable symbol 0669409053 matched a Payments fiobanka row and processed webhook completion without manual DB/status bypass]; [RESOLVED/NARROWED: Fiobanka webhook source no longer accepts arbitrary non-empty x-fio-signature values]; [MISSING: runtime FIO_BANKA_WEBHOOK_SECRET configuration and deployment verification]; [MISSING: official/native Fio Banka callback signature contract if provider-authentic bank-originated signatures are required]` plus provider-specific rollback execution blockers.
