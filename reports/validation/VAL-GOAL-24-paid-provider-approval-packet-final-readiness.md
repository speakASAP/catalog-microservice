...
- Payments: clean `main...origin/main` at `ea4fc71 docs: align goal24 fiobanka verifier hardening`. This supersedes `1458ffe` and `e713bc4` by retaining selected-provider callback evidence, the provider rollback packet, Fiobanka rollback boundary, source-only refund/cancel rollback plan, and source-level Fiobanka HMAC signature hardening while still blocking provider-specific refund/cancel/reversal execution approval, runtime `FIO_BANKA_WEBHOOK_SECRET` configuration/deploy verification, official/native Fio Banka callback signature contract if provider-authentic bank-originated signatures are required, Orders/Warehouse mutation, deploy, migration, and secret output.
...
- Payments `ea4fc71 docs: align goal24 fiobanka verifier hardening`: validates a synthetic Fiobanka bank-transfer payment shape, QR page creation path, owner-approved synthetic completed callback through `/webhooks/fiobanka`, selected-provider callback evidence, provider rollback packet evidence, Fiobanka pre-completion rollback boundary, source-only refund/cancel rollback packet requirements, and source-level Fiobanka HMAC signature hardening. It proves matched-payment completion without manual payment-state bypass and removes arbitrary non-empty signature acceptance from source, but does not prove runtime secret/deploy readiness, official/native bank-originated callback authenticity if required, provider-specific refund/cancel/reversal execution, Orders update, Warehouse cleanup execution, or live bundle checkout safety.
- Orders `a7a6947 docs: record synthetic returned shipment fixture`: records that Orders can express rollback choreography only after Payments proves provider refund/cancel/reversal and the owner-approved runtime packet defines Orders cancellation actor/reason/side-effect acknowledgements plus Warehouse `5175ae0` current-head cleanup/delivery documentation retaining `3043cad` cleanup semantics; it also source-verifies current FlipFlop central Orders UUID propagation into Payments create calls.
...
- Orders `a7a6947 docs: record synthetic returned shipment fixture`: merged source/docs evidence that Orders can express completed, failed, and cancelled payment-status effects, owner-approved cancellation cleanup through the Warehouse `5175ae0` current-head documentation retaining the `3043cad` release/cancel/return matrix, and current FlipFlop central Orders UUID propagation into Payments create calls; it still requires Payments-owned provider refund/cancel/reversal proof before any paid rollback.
- Warehouse `5175ae0 docs: record internal delivery smoke`: `origin/main` retains `0b4c41b` cleanup packet evidence verifying component-line hold, release, fulfill/decrement, cancel, expire, return behavior, and the cleanup operation matrix; the newer internal delivery smoke documentation does not approve paid/provider rollback execution. This resolves/narrows Warehouse lifecycle and operation-selection semantics as source evidence only; approved stock window, max quantity, and live canary remain missing.
...
- Payments `ea4fc71 docs: align goal24 fiobanka verifier hardening`: pushed main records pending Fiobanka QR creation, owner-approved synthetic completed callback through `/webhooks/fiobanka`, selected-provider callback evidence, provider rollback packet evidence, Fiobanka pre-completion rollback boundary, source-only refund/cancel rollback plan, and source-level Fiobanka HMAC signature hardening; Catalog treats this as dependency-gated evidence because it still records `[RESOLVED/NARROWED: owner-confirmed real Fiobanka 1 CZK bank transfer for variable symbol 0669409053 matched a Payments fiobanka row and processed webhook completion without manual DB/status bypass]; [RESOLVED/NARROWED: Fiobanka webhook source no longer accepts arbitrary non-empty x-fio-signature values]; [MISSING: runtime FIO_BANKA_WEBHOOK_SECRET configuration and deployment verification]; [MISSING: official/native Fio Banka callback signature contract if provider-authentic bank-originated signatures are required]` plus provider-specific rollback execution blockers.
...
Decision: the packet is no longer missing basic owner inputs, but the live paid/provider bundle smoke remains blocked by technical hard stops: runtime Fiobanka HMAC secret/deploy verification, official/native Fio Banka callback signature contract if provider-authentic bank-originated signatures are required, Fiobanka refund/cancel/reversal execution path, and named live-run executor/runtime validation owner.
