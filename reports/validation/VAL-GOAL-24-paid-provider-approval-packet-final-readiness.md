...
- Payments: clean `main...origin/main` at `1458ffe docs: reconcile goal24 orders token readiness`. This supersedes `f9d40a4` by merging selected-provider callback evidence, the provider rollback packet, and the Fiobanka rollback boundary while preserving the source-only refund/cancel rollback plan and verifier while still blocking provider-specific refund/cancel/reversal execution approval, runtime Fiobanka secret/deploy verification, provider-native signature evidence if required, or approved provider fixture, Orders/Warehouse mutation, deploy, migration, and secret output.
...
- Provider/method: Fiobanka QR bank-transfer, `applicationId=flipflop-service`, max `300 CZK`, `/webhooks/fiobanka` callback route. Payments `f9d40a4` has synthetic QR and synthetic completed callback evidence; runtime Fiobanka secret/deploy verification and provider-native signature evidence remain missing unless the owner explicitly accepts the synthetic fixture for this one run.
...
Decision: the packet is no longer missing basic owner inputs, but the live paid/provider bundle smoke remains blocked by technical hard stops: runtime-verified/approved Fiobanka completion evidence, Fiobanka refund/cancel/reversal execution path, and named live-run executor/runtime validation owner.
